...
return ( <>
 ...
 {/* Logo */}
 <div className="text-pink-600 text-xl font-bold flex-grow font-mono cursor-pointer">
    <span className="text-3xl">Beauty</span>Hub
  </div>
...
<Link to="/login" className="text-black no-underline">
 ...
 <div className="hidden md:flex align-start text-black hover:text-violet-600 cursor-pointer font-semibold">
   {user?.name ? "Welcome back," : ""} {user?.name || "Hello Welcome"}!{" "}
    {user?.name ? (
   <span
 ...
 <button type="button" className="ms-4 bg-pink-600 py-1 px-3 text-white rounded-lg">Logout</button>
 </span>
 ) : null}
 </div>
...
<li>
 ...
 {" "}
 <button className="ms-4 bg-pink-600 py-1 px-3 text-white rounded-lg">Logout</button>
  </span>
  ) : null}
   </div>
   </ul>
